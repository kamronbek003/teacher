...
import { fetchGroupStudents } from "../services/api"; // api.js dan import
...
const uzWeekdaysShort = ["Dush", "Sesh", "Chor", "Pays", "Jum", "Shan", "Yak"];
const uzMonths = [
    "Yanvar",
    "Fevral",
    "Mart",
...
const getWeeklySchedule = (groups) => {
    ...
    groups.forEach((group) => {
        if (group.status !== "FAOL" || !group.darsJadvali || !group.darsVaqt)
            return;
        const scheduleDays = group.darsJadvali
            .split("/")
            .map((day) => day.trim());
        ...
        scheduleDays.forEach((dayStr) => {
            ...
            if (dayNumInGroupSchedule !== undefined) {
                const targetDay = weekSchedule.find(
                    (d) => d.fullDate.getDay() === dayNumInGroupSchedule
                );
                if (targetDay) {
                    if (
                        !targetDay.classes.some(
                            (c) =>
                                c.groupId === group.id &&
                                c.time === startTimeStr
                        )
    ...
    weekSchedule.forEach((day) => {
        day.classes.sort((a, b) => a.startTimeMinutes - b.startTimeMinutes);
    });
    return weekSchedule;
};
...
const StatCardItem = ({
    ...
    >
        <Card
            className={`!p-4 h-full overflow-hidden relative border border-gray-200/80 dark:border-slate-700/50 ${
                bgColor || "bg-white dark:bg-slate-800"
            } shadow-sm hover:shadow-lg transition-shadow`}
        >
            <div className="flex items-center relative z-10">
                <div
                    className={`flex-shrink-0 p-3 rounded-lg mr-4 ${color} bg-opacity-15`}
                >
...
const LeaderSliderCard = ({ leader }) => (
    <div
        className="relative flex-shrink-0 w-64 h-40 rounded-2xl p-5 overflow-hidden 
        ...
        <div className="relative z-10 flex flex-col justify-between h-full">
            <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                    <p
                        className="font-bold text-lg truncate text-gray-800 dark:text-white"
...
const DashboardScreen = ({
    ...
    const selectedDayClasses = useMemo(() => {
        ...
        if (targetIndex < 0 || targetIndex >= weeklySchedule.length) {
            const todayActualIndex = weeklySchedule.findIndex(
                (day) => day.isToday
            );
    ...
    const totalActiveGroups = useMemo(
        () => groups?.filter((g) => g.status === "FAOL").length || 0,
        [groups]
    );
    ...
    const marqueeVariants = useMemo(() => {
        ...
        const repeatedList = Array.from({
            length: Math.max(1, Math.ceil(8 / leaderTeachers.length)),
        ...
        >
            ...
            >
                <div className="flex justify-between items-center relative z-10">
                    ...
                    <Avatar
                        ...
                        className="h-14 w-14 flex-shrink-0 border-2 border-white/60 shadow-sm"
                    />
                </div>
            </motion.div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
                <div className="lg:col-span-2 space-y-6 md:space-y-8">
                    <Card
                        variants={cardVariants}
                        className="overflow-hidden !p-0 shadow-sm hover:shadow-lg transition-shadow"
                    >
                        <div className="p-5 border-b border-gray-100 dark:border-slate-700/50">
                            <h3 className="text-xl font-semibold text-gray-800 dark:text-gray-100 flex items-center">
                                <CalendarDays className="w-5 h-5 mr-2.5 text-indigo-600 dark:text-indigo-400" />{" "}
                        ...
                        <div className="p-5">
                            ...
                            >
                                {weeklySchedule.map((day, index) => (
                                    ...
                                    >
                                        <p
                                            className={`text-[11px] sm:text-xs font-semibold mb-0.5 ${
                                                selectedDayIndex === index
                            ...
                            <div className="mt-4 min-h-[150px] relative bg-gray-50 dark:bg-slate-800/50 p-4 rounded-lg border border-gray-200 dark:border-slate-700/50">
                                <h4 className="font-semibold text-gray-700 dark:text-gray-300 mb-3 text-base">
                                    {weeklySchedule[selectedDayIndex]
                                        ?.dayName || "Kun tanlanmagan"}
                                    , {weeklySchedule[selectedDayIndex]?.date}{" "}
                                    {weeklySchedule[selectedDayIndex]?.month}
                                </h4>
                                            ...
                                            {selectedDayClasses.map(
                                                (cls, clsIndex) => (
                                                    ...
                                                    >
                                                        ...
                                                        <span className="font-medium text-gray-800 dark:text-gray-100 w-12 tabular-nums">
                                                            {cls.time}
                                                        </span>
                                                        <span
                ...
                <div className="space-y-6 md:space-y-8">
                    <Card
                        variants={cardVariants}
                        className="!p-0 shadow-sm hover:shadow-lg transition-shadow"
                    >
                        <div className="p-5 border-b border-gray-100 dark:border-slate-700/50">
                            <h3 className="text-xl font-semibold text-gray-800 dark:text-gray-100 flex items-center">
                                <TrendingUp className="w-5 h-5 mr-2.5 text-green-600 dark:text-green-400" />{" "}
                                Umumiy Holat
                            </h3>
                        </div>
                    ...
                    <Card
                        variants={cardVariants}
                        className="!p-0 bg-gray-50 dark:bg-slate-900 overflow-hidden shadow-sm hover:shadow-lg transition-shadow"
                    >
                        <div className="p-5 border-b border-gray-200 dark:border-slate-700/50">
                            <h3 className="text-xl font-semibold text-gray-800 dark:text-gray-100 flex items-center">
                                <Award className="w-5 h-5 mr-2.5 text-yellow-500 dark:text-yellow-400" />{" "}
                                Haftaning Lider O'qituvchilari
                            </h3>
                        </div>
                            ...
                            {isLoadingTopTeachers ? (
                                        ...
                                        >
                                            {Array.from({
                                                ...
                                                .map((leaderTeacher, index) => (
                                                    <LeaderSliderCard
                                                        key={`${leaderTeacher.id}-${index}`}
                                                        leader={leaderTeacher}
                                                    />
                                                ))}
                                        </motion.div>
                                    </div>
                                ...
                                <div className="w-full flex flex-col items-center justify-center text-center h-full text-gray-500 dark:text-gray-400">
                                    ...
                                    <p className="text-base font-medium">
                                        Liderlar Hali Tanlanmagan
                                    </p>
                                    <p className="text-sm text-gray-500">
                                        Bu hafta uchun lider o'qituvchilar
                                        topilmadi.
                                    </p>
                                </div>
                            )}
